...
class Ramification(Queue):
    ...
    def extend(self, items):
        self.A.extend(items)
        list.sort(self.A, key=lambda item: item.path_cost)

    def pop(self):
        return self.A.pop(0)


class RamificationHeu(Queue):
    ...
    def extend(self, items):
        self.A.extend(items)
        list.sort(self.A, key=lambda item: item.path_cost + self.problema.h(item))
